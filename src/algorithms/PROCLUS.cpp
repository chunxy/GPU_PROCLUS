...
void greedy(int *M, float *dist, float *new_dist, at::Tensor data, int *S, int Bk, int Ak, int d) {
    ...
    M[0] = S[rnd_start];
    compute_l2_norm_to_medoid_parallel(dist, data, S, M[0], Ak, d);

    for (int i = 1; i < Bk; i++) {
        M[i] = S[argmax_1d(dist, Ak)];
        int m_i = M[i];

        compute_l2_norm_to_medoid_parallel(new_dist, data, S, m_i, Ak, d);
        index_wise_minimum(dist, new_dist, Ak);
    }
}

void greedy_parallel(int *M, float *dist, float *new_dist, at::Tensor data, int *S, int Bk, int Ak, int d) {
    ...
    for (int i = 1; i < Bk; i++) {
        ...
        compute_l2_norm_to_medoid_parallel(new_dist, data, S, m_i, Ak, d);
        index_wise_minimum_parallel(dist, new_dist, Ak);
    }
}
...
bool **find_dimensions_parallel(at::Tensor data, int **L, int *L_sizes, int *M, int k, int n, int d, int l) {
...
int *assign_points_parallel(at::Tensor data, bool **D, int *M, int n, int d, int k) {
    int *C = array_1d<int>(n);
    float **dist = array_2d<float>(n, k);
...
float evaluate_cluster_parallel(at::Tensor data, bool **D, int *C, int n, int d, int k) {
    float **Y = zeros_2d<float>(k, d);
    float *w = array_1d<float>(k);
...
bool *bad_medoids_parallel(int *C, int k, float min_deviation, int n) {
    bool *bad = zeros_1d<bool>(k);
    int *sizes = zeros_1d<int>(k);
...
void remove_outliers_parallel(at::Tensor data, int *C, bool **D, int *M, int n, int k, int d) {
    float *delta = array_1d<float>(k);
...
#pragma omp parallel for
    ...
    float *new_dist = array_1d<float>(n);
    int *M = array_1d<int>(Bk);
    greedy_parallel(M, dist, new_dist, data, S, Bk, Ak, d);
    free(S);
    free(new_dist);
...
#pragma omp parallel for
        ...
        bool **D = find_dimensions_parallel(data, L, L_sizes, M_current, k, n, d, l);
        int *C = assign_points_parallel(data, D, M_current, n, d, k);

        float objective_function = evaluate_cluster_parallel(data, D, C, n, d, k);
        ...
        if (objective_function < best_objective) {
            ...
            bad = bad_medoids_parallel(C, k, min_deviation, n);
        } else {
            free(C);
            free(M_current);
        }
...
#pragma omp atomic
    ...
    bool **D = find_dimensions_parallel(data, L, L_sizes, M_best, k, n, d, l);

    int *C = assign_points_parallel(data, D, M_best, n, d, k);

    remove_outliers_parallel(data, C, D, M_best, n, k, d);
...
bool **
find_dimensions_KEEP_parallel(at::Tensor data, float **H, int *lambda, bool *bad, int **Delta_L, int *Delta_L_sizes,
                     int *L_sizes,
                     int *M, int k, int n, int d, int l) {
    //print_debug("find_dimensions - start\n", DEBUG);
    float **X = array_2d<float>(k, d);
...
std::vector <at::Tensor>
PROCLUS_KEEP_parallel(at::Tensor data, int k, int l, float a, float b, float min_deviation, int termination_rounds, bool debug) {
...
#pragma omp parallel for
    ...
    float *new_dist_tmp = array_1d<float>(n);
    int *M = array_1d<int>(Bk);
    greedy_parallel(M, dist_tmp, new_dist_tmp, data, S, Bk, Ak, d);
    free(S);
    free(dist_tmp);
    free(new_dist_tmp);
...
bool **
find_dimensions_SAVE_parallel(at::Tensor data, float **H, int *lambda, int *M_idx, int **Delta_L, int *Delta_L_sizes,
                     int *L_sizes, int *M_current, int k, int n, int d, int l) {
...
#pragma omp parallel for
    ...
    //# ensuring that we find at least 2 for each and than the k*l #todo fast - sort first instead
#pragma omp parallel for collapse(2)
    for (int i = 0; i < k; i++) {
        for (int _ = 0; _ < 2; _++) {
            int j = argmin_1d<float>(Z[i], d);
...
std::vector <at::Tensor>
PROCLUS_SAVE_parallel(at::Tensor data, int k, int l, float a, float b, float min_deviation, int termination_rounds, bool debug) {
...
#pragma omp parallel for
        ...
        ///changed - end
        bool **D = find_dimensions_SAVE_parallel(data, H, lambda, M_idx, Delta_L, Delta_L_sizes, L_sizes, M_current, k, n, d, l);
        int *C = assign_points_parallel(data, D, M_current, n, d, k);
...
#pragma omp parallel for
                ...
                bool **D = find_dimensions_SAVE_parallel(data, H, lambda, M_idx, Delta_L, Delta_L_sizes, L_sizes, M_current, k,
                                                         n, d, l);
...
#pragma omp parallel for
            ...
            bool **D = find_dimensions_parallel(data, r_L, r_L_sizes, M_best, k, n, d, l);
